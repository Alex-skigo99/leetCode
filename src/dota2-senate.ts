function predictPartyVictory(senate: string): string {
    ...
    function killOrWin(senate: string, current: number): {newSenate: string; newCurr: number; next: boolean} {
        const partyCurrent = senate[current];
        const partyAlter = partyCurrent === 'R' ? 'D' : 'R';
        const len = senate.length;
        let newCurr: number = current;
        for (let y=1; y<len; y++) {
            let i = (current + y) % len;
...
function predictPartyVictory_2(senate: string): string {
    // https://leetcode.com/problems/dota2-s
    // recursive solution
    function killOrWin(senate: string, current: number): string {
        const partyCurrent = senate[current];
        const partyAlter = partyCurrent === 'R' ? 'D' : 'R';
        const len = senate.length;
        let newCurr: number = current;
